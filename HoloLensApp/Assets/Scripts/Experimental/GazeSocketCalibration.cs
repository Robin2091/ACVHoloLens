...
public class GazeSocketCalibration : MonoBehaviour
{
    [SerializeField]
    ...
    private void sendGazeData(int[] screenCoord, DateTime timeStamp, int timeStampLength)
    {
        ...
        string adjustmentAngleFormatted = Math.Round(gazeVectorAngleAdjustment, 2).ToString("#.00");
        byte[] Data = Encoding.UTF8.GetBytes(timeStampFormatted + adjustmentAngleFormatted + formattedEyeTrackingData);
        ...
        if (cameraCoordinateSystem != null)
        {
            ...
            if (eyes != null)
            {
                ...
                if (eyeGaze != null)
                {
                    ...
                    gazeDataQueue.Enqueue(new GazeData(cameraCoordinateSystem, eyeGaze.Value.Origin, eyeGazeDirection));
    ...
    {
        System.Diagnostics.Debug.WriteLine("STOP DATA SENDING");
