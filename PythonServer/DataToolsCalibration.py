...
class DataTools:
    ...
    def generate_images_with_gaze(self, frame_timestamps, gaze_data, folder):
        ...
        for frame in frame_timestamps:
            ...
            for i, gaze_point in enumerate(gaze_data[endpoint:]):
                endpoint += 1
                timestamp, angle, point = gaze_point
                if prev_angle is not None:
                    if angle != prev_angle:
                        rand_color = random.choices(range(256), k=3)
                        image = cv2.putText(image, str(angle), (init_point[0], point[1] + 20),
                                            cv2.FONT_HERSHEY_SIMPLEX, 1, rand_color, 2, cv2.LINE_AA)
                if frame_timestamp > timestamp:
                    cv2.circle(image, point, 10, rand_color, 5)
                else:
    ...
    def find_best_angle(self, focus_point, gaze_data):
        angle_dict = {}
        count = 0
        sum = 0
        prev_angle = None
        for gaze in gaze_data:
            timestamp, angle, point = gaze
            if angle not in angle_dict:
                if prev_angle is not None:
                    angle_dict[prev_angle] = sum / count
                angle_dict[angle] = 0
                count = 0
                sum = 0
            prev_angle = angle
            count += 1
            sum += self.compute_distance(focus_point, point)
        print(angle_dict)
